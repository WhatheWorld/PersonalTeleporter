			...
			for indexE,equip in pairs(equipment) do
				if equip.name == "Personal-Teleporter" then
					...
					if equip.energy > 2000000 then
						...
						local localEntities = game.surfaces["nauvis"].find_entities({{TelporterLocation[2]-1,TelporterLocation[3]+1},{TelporterLocation[2]+1,TelporterLocation[3]-1}})
...
script.on_event(defines.events.on_built_entity, function(event)
	if string.find(event.created_entity.name,"Teleporter_Beacon") then
		...
		table.insert(global.TelaportLocations,{"x:"..x..",y:"..y,x,y})
...
function updateTelporterList()
	for i,telaportLoc in ipairs(global.TelaportLocations) do
      if (i > 0) then
       local localEntities = game.surfaces["nauvis"].find_entities({{telaportLoc[2]-1,telaportLoc[3]+1},{telaportLoc[2]+1,telaportLoc[3]-1}})
